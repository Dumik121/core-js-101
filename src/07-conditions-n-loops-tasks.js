...
function getDigitalRoot(num) {
  return num.toString().split('').map((i) => Number(i)).reduce((a, b) => a + b, 0)
    .toString()
    .split('')
    .map((i) => Number(i))
    .reduce((a, b) => a + b, 0);
...
function getMatrixProduct(m1, m2) {
  ...
  for (i = 0; i < m1.length; i += 1) {
    ...
    for (j = 0; j < m2[i].length; j += 1) {
      res[i][j] = 0;
      for (k = 0; k < m1[i].length; k += 1) res[i][j] += m1[i][k] * m2[k][j];
    }
  }
  return res;
}
...
function evaluateTicTacToePosition(position) {
  const checkRow = (row) => row[0] && row[0] === row[1] && row[0] === row[2];

  const checkColumn = (colIndex) => position[0][colIndex]
    && position[0][colIndex] === position[1][colIndex]
    && position[0][colIndex] === position[2][colIndex];

  const checkDiagonals = () => position[1][1]
    && ((position[0][0] === position[1][1] && position[0][0] === position[2][2])
      || (position[0][2] === position[1][1] && position[0][2] === position[2][0]));

  // Check rows
  if (checkRow(position[0])) return position[0][0];
  if (checkRow(position[1])) return position[1][0];
  if (checkRow(position[2])) return position[2][0];

  // Check columns
  if (checkColumn(0)) return position[0][0];
  if (checkColumn(1)) return position[0][1];
  if (checkColumn(2)) return position[0][2];

  // Check diagonals
  if (checkDiagonals()) return position[1][1];
